...
Friday wrap uses project `wdyzzxrqphfmroxcskna` (`https://wdyzzxrqphfmroxcskna.supabase.co`). The `profiles` and `habit_events` schema in `supabase/migrations/001_profiles_friday_wrap.sql` is already applied there. `002_lock_periods.sql` adds the 12-month unlock on each paid lock. `003_gifts.sql` records a gift against the chosen recipient, not the sender's lock. Apply those files on `wdyzzxrqphfmroxcskna` only. Do not run them on any other project, including `etshbvpvhgnifcejptay`.
